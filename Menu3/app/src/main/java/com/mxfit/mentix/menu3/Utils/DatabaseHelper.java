...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public boolean insertMultipleData(LatLng[] locations, int size, double distance, double time){
        db = this.getWritableDatabase();
    ...
    public Cursor queryPass(String string){
        SQLiteDatabase db = this.getWritableDatabase();
        return db.rawQuery("SELECT * FROM "+string + " where ROWID = 1", null);
    }

    public Cursor selectSpecificTable(String tablename){
        SQLiteDatabase db = this.getWritableDatabase();
