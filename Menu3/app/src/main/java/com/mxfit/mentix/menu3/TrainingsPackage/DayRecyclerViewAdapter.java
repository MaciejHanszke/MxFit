...
class DayRecyclerViewAdapter extends RecyclerView.Adapter<DayRecyclerViewAdapter.ViewHolder> {
    ...
    DayRecyclerViewAdapter(List<PreRun> preRuns, List<TrainingUnit> pushups, DayChoiceActivity pPCA)
    {
        mPreRuns = preRuns;
        mPushups = pushups;
        PCA = pPCA;
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, final int position) {
        ...
        final TrainingUnit trainingUnit = mPushups.get(position);
            ...
            String text = "<font color=#00a6ff>"+ trainingUnit.getSets() +"</font>,  " + preRun.getKm();
    ...
    private void showDialog(final int day, final float goal){
        if(GlobalValues.lastPushupDate == null && GlobalValues.lastRunDate == null)
        {
            GlobalValues.rday = day;
            GlobalValues.goal = goal;
            PCA.setChosen();
            PCA.exit();
        } else {
